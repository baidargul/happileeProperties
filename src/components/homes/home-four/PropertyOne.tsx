...
const PropertyOne = async ({ style_1, style_2 }: any) => {
  ...
  // JUST FOR TESTING AND PRINTING THE FIRST PROPERTY
  if (properties.length > 0) {
    console.log(properties[0]);
  }

  // PLAY WITH Properties Now
        ...
        <div className="position-relative">
          ...
          <div id="myCarousel" className="f-carousel">
            {property_data?.map((item) => (
              <div
                key={item.id}
                className="col-sm d-flex mt-40 wow fadeInUp mx-auto f-carousel__slide"
                style={{ margin: "10px 5px" }}
              >
                <div
                  className="listing-card-one w-100 style-two shadow-none h-100 w-100 bg-light"
